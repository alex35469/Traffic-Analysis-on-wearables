...
all_columns = set()
def extract_columns(dataset_file, all_columns):
...
def find_common_columns(sources_files):
    ...
    for source in sources_files:
        all_columns = extract_columns(source, all_columns)
...
def print_deep_packet_counts(events_train, counts_train, events_test, counts_test):
    col_width = 20
    def _pad(s):
        return pad(s, col_width)

    def print_row(*values):
        values = list(values) # value is a tuple which does not support deletion
        train = values[-2]
        test = values[-1]
        train_test = str(train) + "/" + str(test)

        del values[-1] # delete train/test, we'll pad this as one
        del values[-1]
        values.append(train_test)

        values_str = " ".join([_pad(str(v)) for v in values])
        if int(train) < int(test):
            values_str = bcolors.FAIL + values_str + bcolors.ENDC
        elif int(train) < 2 * int(test):
            values_str = bcolors.WARNING + values_str + bcolors.ENDC
        print(values_str)

    line = "-" * col_width

    print("Packet counts:")
    print(_pad("DEVICE"),_pad("APP"),_pad("ACTION"), "#Train/#Test")
    accu3_train, accu3_test = 0, 0
    for device in counts_train:
        accu2_train, accu2_test = 0, 0
        for app in counts_train[device]:
            accu1_train, accu1_test = 0, 0
            for action in counts_train[device][app]:
                print_row(device,app,action,counts_train[device][app][action],counts_test[device][app][action])
                accu1_train += counts_train[device][app][action]
                accu1_test += counts_test[device][app][action]

            print_row(device,app, line, accu1_train, accu1_test)
            accu2_train += accu1_train
            accu2_test += accu1_test

        print_row(device, line, line, accu2_train, accu2_test)
        accu3_train += accu2_train
        accu3_test += accu2_test
    print_row("Global", line, line, accu3_train, accu3_test)
